...
int main() {
  ...
  for (i = 0; i < MAX_CUST; i++) {
    status =
        pthread_create(&customer[i], NULL, (void *)customerThread, (void *)&i);
    wait();
    if (status != 0) {
...
void customerThread(void *tmp) {
  ...
  if (numberOfFreeSeats > 0) {
    ...
    sem_post(&mutex);
    int satisfaction =
        rand() % 5 + 1; // Generate a random satisfaction level (1-5)
    customerFeedbackData[B].customerID = B;
    customerFeedbackData[B].satisfactionLevel = satisfaction;
    printf("Customer-%d Provides Feedback: Satisfaction Level %d\n", B,
           satisfaction);
  } else {
    sem_post(&mutex);
