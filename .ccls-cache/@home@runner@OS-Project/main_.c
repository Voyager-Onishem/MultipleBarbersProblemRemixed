...
int main()
{
    ...
    for (i = 0; i < MAX_CUST; i++)
    {
        ...
        int x = rand() % 6; // Fixing the array index out of bounds issue
        sleep(rand() % 5);
        if (status != 0)
        {
...
void customerThread(void *tmp)
{
    ...
    if (numberOfFreeSeats > 0)
    {
        ...
        printf("Customer-%d is waiting for barber recognition.\n\n", count);

        // Wait until the barber acknowledges the customer
        sem_wait(&mutex);
        sem_post(&mutex);
...
void managementThread(void *tmp)
{
    ...
    while (1)
    {
        ...
        if (insuff || (numberOfFreeSeats < 10 && CURR_BARB < 5 && !(MAX_CHAIRS == numberOfFreeSeats)))
        {
